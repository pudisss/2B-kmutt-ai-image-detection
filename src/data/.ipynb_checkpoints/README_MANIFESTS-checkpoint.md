...
This directory contains manifest CSV files that allow you to have full control over which data samples are used for training, validation, and testing.

## Manifest Files

- **train_manifest.csv**: Samples used for training
- **val_manifest.csv**: Samples used for validation during training
...
python scripts/train.py \
    ...
    --val-manifest /path/to/custom_val.csv \
...
1. **Full Control**: You decide exactly which samples go into train/val/test
...
- Validation manifest is optional; if not provided, it will be created by splitting the training manifest (10% by default)
