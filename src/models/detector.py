...
class MultiDomainDetector(nn.Module):
    ...
    def __init__(
        ...
        self.noise_branch = NoiseBranch(
            ...
            resnet_type=noise_resnet,
            pretrained=True,
            use_srm_layer=False,  # Pre-computed residuals from dataset
        )

        # Per-domain post-extraction normalization
        self.rgb_norm = nn.LayerNorm(rgb_feature_dim)
        self.freq_norm = nn.LayerNorm(freq_feature_dim)
        self.noise_norm = nn.LayerNorm(noise_feature_dim)

        # Project domain features to a shared space for cosine similarity.
        self.sim_projection_dim = min(rgb_feature_dim, freq_feature_dim, noise_feature_dim)
        self.rgb_sim_projection = nn.Linear(rgb_feature_dim, self.sim_projection_dim)
        self.freq_sim_projection = nn.Linear(freq_feature_dim, self.sim_projection_dim)
        self.noise_sim_projection = nn.Linear(noise_feature_dim, self.sim_projection_dim)
        
        # Create fusion module
        # Append three pairwise cosine similarities to each domain feature.
        self.similarity_dim = 3
        branch_dims = [
            rgb_feature_dim + self.similarity_dim,
            freq_feature_dim + self.similarity_dim,
            noise_feature_dim + self.similarity_dim,
        ]
        ...
        self.config = {
            'rgb_model': rgb_model,
            'freq_resnet': freq_resnet,
            'noise_resnet': noise_resnet,
            'fusion_type': fusion_type,
            'num_classes': num_classes,
        }
    ...
    def forward(
        ...
        # LayerNorm per domain after feature extraction
        rgb_feat = self.rgb_norm(rgb_feat)
        freq_feat = self.freq_norm(freq_feat)
        noise_feat = self.noise_norm(noise_feat)

        # L2-normalize for cosine-similarity computations
        rgb_sim = self.rgb_sim_projection(rgb_feat)
        freq_sim = self.freq_sim_projection(freq_feat)
        noise_sim = self.noise_sim_projection(noise_feat)

        rgb_l2 = F.normalize(rgb_sim, p=2, dim=1)
        freq_l2 = F.normalize(freq_sim, p=2, dim=1)
        noise_l2 = F.normalize(noise_sim, p=2, dim=1)

        # Pairwise cosine similarities between domains [B, 1] each
        sim_rf = F.cosine_similarity(rgb_l2, freq_l2, dim=1).unsqueeze(1)
        sim_rn = F.cosine_similarity(rgb_l2, noise_l2, dim=1).unsqueeze(1)
        sim_fn = F.cosine_similarity(freq_l2, noise_l2, dim=1).unsqueeze(1)
        sim_features = torch.cat([sim_rf, sim_rn, sim_fn], dim=1)  # [B, 3]

        # Concatenate similarity features and pass into default fusion flow
        rgb_fusion_feat = torch.cat([rgb_feat, sim_features], dim=1)
        freq_fusion_feat = torch.cat([freq_feat, sim_features], dim=1)
        noise_fusion_feat = torch.cat([noise_feat, sim_features], dim=1)
        
        # Fuse features
        fused = self.fusion([rgb_fusion_feat, freq_fusion_feat, noise_fusion_feat])
        ...
        if return_features:
            ...
            output['similarity_features'] = sim_features
...
def create_detector(
    ...
    **kwargs,
    ...
    # Default config
    config = {
        'rgb_model': 'microsoft/swin-tiny-patch4-window7-224',
        'rgb_pretrained': True,
        'rgb_feature_dim': 512,
        'freq_feature_dim': 256,
        'freq_resnet': 'resnet18',
        'noise_feature_dim': 256,
        'noise_resnet': 'resnet18',
